...
def minimum_phase(h, method='homomorphic', n_fft=None):
    ...
    >>> for h, static, color in zip((h_linear, h_min_hom, h_min_hil),
    ...                            ('-', '-', '--'), ('k', 'r', 'c')):
    ...     w, H = freqz(h)
    ...     w, gd = group_delay((h, 1))
    ...     w /= np.pi
    ...     axs[0].plot(h, color=color, linestyle=static)
    ...     axs[1].plot(w, np.abs(H), color=color, linestyle=static)
    ...     axs[2].plot(w, 20 * np.log10(np.abs(H)), color=color, linestyle=static)
    ...     axs[3].plot(w, gd, color=color, linestyle=static)
    >>> for ax in axs:
